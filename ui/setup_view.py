...
from dart_cup import SPIEL_MODI, PUNKTE_VARIANTEN
...
def create_setup_ui(game):
    ...
    game.variant_box.addItems(PUNKTE_VARIANTEN)
    mode_layout.addWidget(game.variant_box)

    def on_mode_changed(text):
        modus_daten = SPIEL_MODI.get(text)
        aktiv = modus_daten[2] if modus_daten and len(modus_daten) > 2 else True
    ...
    game.player_inputs = []
    game.bot_types = []  # Hier landen die ComboBoxen für die main_window.py

    # 8 Spieler generieren: Name + Bot-Zuweisung nebeneinander
    for i in range(8):
        row = i // 2
        col_offset = (i % 2) * 2  # Spalte 0 & 1 für linke Seite, 2 & 3 für rechte Seite

        line_edit = QLineEdit()
        line_edit.setPlaceholderText(f"Spieler {i+1}")
        line_edit.setFixedWidth(180)  # Leicht verkleinert, damit das Dropdown daneben passt
